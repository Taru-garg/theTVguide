...
def get_data(API_key, TV_ID):
    query = 'https://api.themoviedb.org/3/tv/'+str(TV_ID)+'?api_key='+API_key+'&language=en-US'
...
def write_file(filename, text,tv):
    ...
    try:
        ...
        #for movies that don't belong to a collection, assign null
        last_air_date = None            
    result = [tv,dataset['original_name'],overview,genre,first_air_date,last_air_date,no_of_seasons,no_of_episodes,backdrop_path,poster_path]
    # write data
...
csvwriter = csv.writer(csvFile)
csvwriter.writerow(['ID','TV_show_name','Overview','Genres','First_Air_Date','Last_Air_Date','number_of_seasons','number_of_episodes','backdrop_path','poster_path'])
csvFile.close()
